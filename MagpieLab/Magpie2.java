public class Magpie2
{
	...
	{
		MagpieRunner2 var = new MagpieRunner2();
		String response = "";
		...
		{
			int negResponse = findKeyword(statement, "no", 0);
			if(negResponse == 1){
				...
				|| statement.indexOf("turtle") >= 0)
		{
			...
			response = "He sounds like a brilliant teacher";
		}
		else
		{
			response = getRandomResponse();
	...
	private int findKeyword(String statement, String goal, int startPos)
	{
		...
		String pre_phrase = statement.toLowerCase();
		String phrase = pre_phrase.trim();
		String before = "";
		String after = "";
		int psn = phrase.indexOf(goal, startPos);
		while(psn >= 0){
			if(psn > 0){
				before = phrase.substring(0, startPos);
				return 2;
			}
			else if(psn == 0){
